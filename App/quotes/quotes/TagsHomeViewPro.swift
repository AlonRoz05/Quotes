                        ...
                        LazyVGrid(columns: gridItems, spacing: 20) {
                            if let tags = tagData?.tags {
                                ForEach(tags, id: \.self) { tag in
                                    TagView(TagName: tag, viewModel: viewModel, isSelected: tag == viewModel.selectedTag, isPro: true)
                                        .onTapGesture {
                                            viewModel.addTagForPro(tagToSelect: tag)
                                        }
                                }
                            } else {
